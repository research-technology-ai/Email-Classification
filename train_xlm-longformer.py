...
class GeneralisticDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self):
        ...
        def tokenize_dataset(dataset, tokenizer):
            def tokenize_and_align_labels(examples):
                    tokenized_inputs = tokenizer(examples['token_classification']["tokens"], is_split_into_words=True, truncation=True, max_length=4096)
                    example_choices = [0] * len(choices)
            ...
            for example in dataset:
                ...
                new_dataset.append({'dataset_id': example['id'], 'personal_id': 0, 'input_ids':tokenized_example['input_ids'], 'attention_mask':tokenized_example['attention_mask'], 'labels_choices':tokenized_example['labels_choices']})
            return new_dataset
        ...
        if self.already_called:
            ...
            tokenizer = AutoTokenizer.from_pretrained(self.model, truncation=True, max_length=4096)
            for file in ['train', 'val', 'test']:
                ...
                with open(f'./data/{file}_tokenized_xlm-longformer.json', 'w') as f:
    ...
    def setup(self, stage=None):
            ...
            dataset = load_dataset('json', data_files={'train': f'./data/train_tokenized_xlm-longformer.json', 'validation': f'./data/val_tokenized_xlm-longformer.json', 'test': f'./data/test_tokenized_xlm-longformer.json'})
    ...
    def cleanup(self, stage=None):
        ...
        for file in ['train', 'val', 'test']:
            os.remove(f'./data/{file}_tokenized_xlm-longformer.json')
